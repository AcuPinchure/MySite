...
def getStats(request):
    ...
    try:
        ...
        tweet_q = Tweet.objects.filter(media__seiyuu__id_name=seiyuu, post_time__gte=start_date,
                                       post_time__lte=end_date, data_time__isnull=False).order_by("data_time")
        follower_q = Followers.objects.filter(
            seiyuu__id_name=seiyuu, data_time__gte=start_date, data_time__lte=end_date).order_by("data_time")
    except TypeError:
    ...
    top_like_id_list = tweet_q.order_by(
        '-like')[:10].values_list("id", flat=True)
